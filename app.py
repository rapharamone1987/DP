...
# --- 2. FUNÇÃO DE EXTRAÇÃO COM CACHE (O SEGREDO CONTRA O ERRO 429) ---
@st.cache_data(show_spinner=False)
def extrair_dados_pdf_cache(pdf_bytes, natureza_tipo):
    """
    Esta função salva o resultado. Se o Streamlit reiniciar, 
    ele não chama a IA de novo, ele pega o que guardou aqui.
    """
    try:
        time.sleep(2) # Respiro para a API não entender como spam
        prompt = f"Analise o PDF para recebimento {natureza_tipo}. EXTRAIA MARCA, MODELO e ITENS REAIS. Formato JSON: FORNECEDOR: x, EDITAL: x, OBJETO: x, CHECKLIST: - item"
        
        # Chamada da IA
        res = model.generate_content([{'mime_type': 'application/pdf', 'data': pdf_bytes}, prompt])
        
        # Parser manual para garantir que o texto vire dados úteis
        texto = res.text
...
def desenhar_check(pdf, x, y, status):
    if status:
        ...
        pdf.set_draw_color(255, 255, 255); pdf.line(x+1.2, y+2.5, x+2.2, y+3.8); pdf.line(x+2.2, y+3.8, x+3.8, y+1.5)
    else:
        pdf.set_fill_color(200, 0, 0); pdf.ellipse(x, y, 5, 5, 'F')
        pdf.set_draw_color(255, 255, 255); pdf.line(x+1.5, y+1.5, x+3.5, y+3.5); pdf.line(x+3.5, y+1.5, x+1.5, y+3.5)
...
# --- 4. INTERFACE ---
st.set_page_config(page_title="Recebimento IA", layout="centered")
...
if not st.session_state.checklist_items:
    ...
    if pdf_file:
        if st.button("🔍 ANALISAR DOCUMENTO (Gasta 1 crédito de IA)"):
            with st.spinner("IA extraindo itens..."):
                # Passamos o conteúdo do PDF e a natureza para a função com CACHE
                resultado = extrair_dados_pdf_cache(pdf_file.read(), natureza)
                
                if isinstance(resultado, dict):
                    st.session_state.dados_auto = resultado
                    st.session_state.checklist_items = resultado["checklist"]
                    st.session_state.natureza_escolhida = natureza
                    st.rerun()
                else:
                    if "429" in resultado:
                        st.error("⚠️ Cota Diária Esgotada. Tente novamente amanhã ou use outra chave API.")
                    else:
                        st.error(f"Erro: {resultado}")

# --- 6. CHECKLIST E CONFERÊNCIA ---
if st.session_state.checklist_items:
    obj_curto = " ".join(st.session_state.dados_auto.get("objeto", "BEM").split()[:5]).upper()
    st.markdown(f'<p class="titulo-verde">CHECKLIST: {obj_curto}</p>', unsafe_allow_html=True)
    ...
    with st.container():
        ...
        centro_custo = c2.text_input("Centro de Custo:") if "Permanente" in st.session_state.natureza_escolhida else ""
        st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="barra">1. ITENS TÉCNICOS EXTRAÍDOS</div>', unsafe_allow_html=True)
    ...
    if st.button("🚀 GERAR PDF FINAL"):
        if not servidor: st.error("Informe o servidor.")
            ...
            try:
                ...
                pdf.cell(170, 8, tr(f"EDITAL: {edital}"), ln=True, border='B')
                pdf.write(8, tr("FORNECEDOR: ")); pdf.multi_cell(140, 8, tr(fornecedor.upper()))
                pdf.set_font("Arial", 'B', 10); pdf.cell(170, 8, tr(f"ID: {placa.upper()}"), ln=True)
                if centro_custo: pdf.write(8, tr("C. CUSTO: ")); pdf.multi_cell(140, 8, tr(centro_custo.upper()))
                
                pdf.ln(5); pdf.set_fill_color(0, 154, 68); pdf.set_text_color(255, 255, 255)
                pdf.cell(170, 10, tr(" 1. CONFERÊNCIA REALIZADA"), ln=True, fill=True)
                ...
                for idx, item_txt in enumerate(st.session_state.checklist_items):
                    ...
                    pdf.multi_cell(160, 7, tr(item_txt))
                ...
                if todos_ok:
                    ...
                    t = "RECEBIMENTO DEFINITIVO" if "Consumo" in st.session_state.natureza_escolhida else "RECEBIMENTO PROVISÓRIO"
                    pdf.multi_cell(170, 10, tr(f"ATESTO O {t} DO OBJETO POR ESTAR EM CONFORMIDADE TÉCNICA."), border=1, align='C', fill=True)
                else:
                    pdf.set_font("Arial", 'B', 10); pdf.set_text_color(200, 0, 0)
                    pdf.multi_cell(170, 8, tr(f"PENDÊNCIAS DETECTADAS:\n{obs_geral}"), border=1)

                pdf.ln(20); pdf.set_text_color(0, 0, 0)
                pdf.cell(170, 8, "________________________________________", ln=True, align='C')
                pdf.cell(170, 6, tr(f"SERVIDOR: {servidor.upper()}"), ln=True, align='C')
