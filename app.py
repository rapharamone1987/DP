...
with menu[1]:
    ...
    if st.button("Salvar"):
        if up_b and nome_b:
            ...
            try:
                with open(caminho_salvar, "wb") as f: f.write(up_b.getbuffer())
                st.success(f"Salvo como: {nome_limpo}.pdf")
            except Exception as e: st.error(f"Erro: {e}")
    st.write("---")
    if os.path.exists(banco_atas):
        for f_name in [f for f in os.listdir(banco_atas) if f.endswith(".pdf")]:
            ...
            if c3.button("🗑️", key=f"d_{f_name}"): 
                os.remove(os.path.join(banco_atas, f_name)); st.rerun()
