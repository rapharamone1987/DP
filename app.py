...
# ==========================================
# 1. INICIALIZAÇÃO SEGURA DO ESTADO (BLINDAGEM)
# ==========================================
...
# CONFIGURAÇÃO DA IA (PRIORIDADE MODELOS SUPERIORES)
key = st.secrets.get("GROQ_API_KEY", "")
client = Groq(api_key=key) if key else None

@st.cache_resource
def obter_melhor_modelo():
    """Tenta carregar o modelo 405B (Superior) ou 70B (Versatile)"""
    if not client: return None
    try:
        modelos = client.models.list()
        ids = [m.id for m in modelos.data]
        # Ordem de Superioridade
        preferencia = ["llama-3.1-405b-reasoning", "llama-3.3-70b-versatile", "llama-3.1-70b-versatile"]
...
def extrair_dados_ia(texto_entrada):
    prompt = (
        "Você é um Auditor Técnico de Recebimento de Materiais. "
        "Analise o texto e extraia detalhes técnicos FÍSICOS REAIS (Marca, modelo, peças, medidas, voltagem, etc). "
        "IGNORE cláusulas jurídicas e obrigações da empresa. Extraia o dado REAL do PDF. "
...
if not st.session_state.items_lista:
    ...
    with tabs[0]:
        ...
        if pdf_file and st.button("🔍 Analisar Documento"):
            with st.spinner(f"IA ({MODELO_ID}) analisando..."):
                texto = ""
    ...
    with tabs[1]:
        ...
        if st.button("🚀 Gerar do Texto") and txt_input:
...
# --- TELA DE OPERAÇÃO ---
    ...
    with st.container(border=True):
        ...
        st.session_state.cabecalho["placa"] = c1.text_input("ID/Placa:", value=st.session_state.cabecalho.get("placa",""))
