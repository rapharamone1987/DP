...
def perguntar_ia(pergunta, contexto):
    if client and contexto:
        prompt = f"Baseado no documento, responda de forma curta: {pergunta}\n\nDocumento:\n{contexto}"
...
if not st.session_state.items_lista:
    ...
    pdf_file = st.file_uploader("Selecione o arquivo PDF (TR, ARP ou Empenho)", type="pdf")
        ...
        if pdf_file:
            with st.spinner("IA extraindo dados técnicos..."):
                ...
                if res:
                    ...
                    st.error("A IA falhou ao processar os dados automáticos. Tente novamente ou use a entrada manual.")
        else:
            st.warning("Selecione um arquivo PDF primeiro.")

    if col2.button("🖊️ ENTRADA MANUAL"):
        if pdf_file:
            with st.spinner("Lendo PDF para suporte do Chat..."):
                with pdfplumber.open(pdf_file) as pdf:
...
# --- TELA OPERACIONAL ---
    ...
    if st.session_state.texto_pdf:
        with st.expander("🤖 Chat Suporte (Dúvidas sobre o PDF)"):
            p_user = st.text_input("Qual sua dúvida sobre o documento?")
    ...
    with st.container(border=True):
        ...
        unidade = c2.text_input("Unidade Destino:")
    ...
    for i, itm in enumerate(st.session_state.items_lista):
        ...
        with st.container(border=True):
            ...
            # Uso de .get() para evitar erro se a chave sumir
            ...
            itm["texto"] = col_tx.text_input(f"txt_{uid}", value=itm["texto"], key=f"in_{uid}", label_visibility="collapsed")
