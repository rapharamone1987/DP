...
# CONFIGURAÇÃO DA IA (MODELO 70B - ALTO DESEMPENHO)
...
def extrair_dados_ia(texto_entrada):
    # PROMPT OTIMIZADO CONTRA ALUCINAÇÕES E LENTIDÃO
    prompt = (
        "Você é um extrator de dados técnicos de hardware. Sua única função é localizar "
        "informações FÍSICAS no texto. \n"
        "REGRAS:\n"
        "1. IGNORE cláusulas de multas, pagamentos, vigência, prazos ou documentos.\n"
        "2. EXTRAIA: Marca, Modelo, Dimensões, Potência, Tensão (V), Cor, Peças e Acessórios.\n"
        "3. Se não encontrar o dado real, responda 'Não localizado' no campo.\n"
        "4. Responda APENAS o JSON no formato abaixo:\n"
        '{"fornecedor": "nome", "edital": "número da ARP", "objeto": "nome do item", "checklist": ["especificação física 1", "especificação física 2"]}'
    )
    if client:
        try:
            res = client.chat.completions.create(
                ...
                temperature=0.1 # Menos criatividade = Menos alucinação
            )
...
if not st.session_state.items_lista:
    ...
    with tabs[0]:
        ...
        if pdf_file and st.button("🔍 ANALISAR DOCUMENTO"):
            with st.spinner("IA extraindo especificações físicas..."):
                texto = ""
                with pdfplumber.open(pdf_file) as pdf:
                    # Limita a 4 páginas para ser muito mais rápido e evitar lixo administrativo
                    for pg in pdf.pages[:4]: texto += (pg.extract_text() or "") + "\n"
    ...
    with tabs[2]:
        if st.button("🖊️ Iniciar em Branco"):
            st.session_state.items_lista = [{"id": time.time(), "texto": "Conferir Marca e Modelo"}]
            st.rerun()

elif st.session_state.items_lista:
    if st.session_state.texto_pdf:
        with st.expander("🤖 Chat Suporte (Confirmar detalhes no PDF)"):
            p_user = st.text_input("Qual detalhe você quer confirmar?")
...
if st.sidebar.button("Reset Total"): st.session_state.clear(); st.rerun()
