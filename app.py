...
pdf_file = st.file_uploader("Upload do Termo de Referência (TR)", type="pdf")
...
if st.session_state.checklist_items:
    ...
    with st.container():
        ...
        edital = col1.text_input("Edital:", value=st.session_state.dados_auto["edital"])
        processo = col2.text_input("Processo:", value=st.session_state.dados_auto["processo"])
        fornecedor = st.text_input("Fornecedor:", value=st.session_state.dados_auto["fornecedor"])
        placa = st.text_input("Placa / ID do Bem:")
    ...
    st.markdown('<div class="barra-secao">1. ITENS TÉCNICOS DE CONFERÊNCIA</div>', unsafe_allow_html=True)
    ...
    for i, item in enumerate(st.session_state.checklist_items):
        with st.container(border=True):
            ...
            if not st.session_state.conferidos[i]:
                st.session_state.observacoes[i] = st.text_area(f"Descreva a Pendência {i+1}", key=f"obs_{i}")

    st.markdown('<div class="barra-secao">2. IDENTIFICAÇÃO DO SERVIDOR</div>', unsafe_allow_html=True)
    serv_nome = st.text_input("Nome do Servidor que está atestando:")
    ...
    if st.button("🚀 GERAR PDF"):
        try:
            ...
            pdf.multi_cell(170, 10, f"CHECKLIST DE RECEBIMENTO TECNICO - {st.session_state.dados_auto['objeto'].upper()}", align='C')
            pdf.ln(5)
            ...
            # Cabeçalho de Dados com quebra de linha inteligente
            pdf.set_font("Arial", 'B', 10)
            pdf.set_text_color(0, 0, 0)
            
            pdf.cell(85, 8, f"EDITAL/TR/ARP: {edital}")
            
            # Para Fornecedor e Centro de Custo, usamos multi_cell para não estourar
            pdf.write(8, "FORNECEDOR: ")
            pdf.set_font("Arial", '', 10)
            pdf.multi_cell(170, 8, fornecedor.upper())
            
            pdf.set_font("Arial", 'B', 10)
            pdf.cell(170, 8, f"PLACA / ID: {placa.upper()}", ln=True)
            
            if natureza == "Permanente":
                pdf.set_font("Arial", 'B', 10)
                pdf.write(8, "CENTRO DE CUSTO: ")
                pdf.set_font("Arial", '', 10)
                pdf.multi_cell(170, 8, centro_custo.upper())
            ...
            pdf.cell(170, 10, " 1. CONFERENCIA TECNICA E FISICA", ln=True, fill=True)
            pdf.set_text_color(0, 0, 0)
            pdf.ln(5)
            ...
            for idx, item_txt in enumerate(st.session_state.checklist_items):
                ...
                pdf.set_font("Arial", 'B', 10)
                pdf.multi_cell(170, 7, f"{status} {item_txt.encode('latin-1','ignore').decode('latin-1')}")
                
                if not st.session_state.conferidos.get(idx):
                    pdf.set_font("Arial", 'I', 9)
                    pdf.set_text_color(200, 0, 0)
                    pdf.multi_cell(160, 6, f"     OBS: {st.session_state.observacoes.get(idx, 'Pendente')}")
                    pdf.set_text_color(0, 0, 0)
            ...
            if todos_ok:
                ...
                pdf.set_font("Arial", 'B', 10)
                if natureza == "Permanente":
                    texto = "ATESTO O RECEBIMENTO PROVISORIO do bem, conforme Art. 140, I, 'a' da Lei 14.133/21, para posterior verificacao de conformidade tecnica."
                else:
                    texto = "ATESTO O RECEBIMENTO DEFINITIVO dos materiais, por estarem em total conformidade com as especificacoes."
                pdf.multi_cell(170, 8, texto, border=1, align='C', fill=True)
            else:
                pdf.set_font("Arial", 'B', 10)
                pdf.set_text_color(200, 0, 0)
                pdf.multi_cell(170, 8, "DOCUMENTO ENCERRADO COM PENDENCIAS. O ATESTO NAO FOI REALIZADO.", border=1, align='C')
                pdf.set_text_color(0, 0, 0)
            ...
            pdf.cell(170, 10, f"Gerado em: {datetime.now().strftime('%d/%m/%Y %H:%M')}", ln=True, align='C')

            pdf_bytes = pdf.output(dest='S').encode('latin-1', errors='replace')
            st.download_button("📥 Baixar Relatório PDF", data=pdf_bytes, file_name="Relatorio_Tecnico.pdf")
